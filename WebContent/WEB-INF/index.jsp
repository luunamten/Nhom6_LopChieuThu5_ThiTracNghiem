...
<c:if test="${not empty sessionScope.loginBean}">
	<c:set var="utype" value="${sessionScope.loginBean.userType}" />
	<c:choose>
		<c:when test="${utype == 'gv'}">
			<jsp:forward page="teacher/teacher.jsp" />
		</c:when>
		<c:when test="${utype == 'sv'}">
			<c:redirect url="student/student.jsp" />
		</c:when>
		<c:when test="${utype == 'qt'}">
